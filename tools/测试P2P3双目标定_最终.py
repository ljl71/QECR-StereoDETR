"""Test QECR P2/P3 conversion without requiring OpenCV/scikit-image at import."""
...
# The test calls only the calibration method.  Lightweight stubs keep optional
# image dependencies from obscuring this CPU-only mathematical check.
...
def main():
    ...
    original_depth = 20.0
    crop_scale = 1.2
    disparity_from_qecr = network_fb / (original_depth * crop_scale)
    disparity_from_affine_image = (
        focal
        * baseline
        / original_depth
        * 1280.0
    ...
    assert abs(
        disparity_from_qecr - disparity_from_affine_image
    ) < 1.0e-5
    ...
    print("P2/P3 基线、随机裁剪深度尺度和极线方向测试通过")
