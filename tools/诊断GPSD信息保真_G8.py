...
def 生成判断(
    ...
    information_pass = (
        ...
        and best["Car_Oracle误差相对下降"] >= 0.10
    )
    if not evidence_ready:
        ...
        status = "通过，允许V23短程配对"
        explanation = (
            "四组相关保留了均匀平均丢失的显著信息。下一步只运行零初始化"
            "RDSA与GPSD同起点配对，不直接进行完整训练。"
        )
    else:
        status = "否决"
        explanation = "分组Oracle上限没有同时达到命中率和误差门槛，停止V23。"
...
def 写出结果(
    ...
    lines = [
        ...
        "- 即便通过，本报告也只授权V23O/A/B各5轮同起点配对，不授权直接跑195轮。",
        "",
        "## 最佳组合分层结果",
        "",
    ...
    lines.extend(
        [
            ...
            "## 证据门槛",
            "",
            "正式判断要求：至少100个样本、s4四组至少10000个目标框内点和",
            "8000个Car点。目标点还必须同时落入变换后2D框，且与目标中心",
            "深度相差不超过 max(2m, 15%)，以排除框内背景污染。",
            "且 Oracle 的 ±1 bin 命中率提升不少于 5 个百分点、平均 bin 误差",
            "相对下降不少于10%；Car还需分别达到3个百分点和10%。",
            "四项必须同时满足。",
            "",
            "## 基线等价性审计",
            "",
...
def 自检() -> None:
    ...
    assert 生成判断(mock, 100)["状态"] == "通过，允许V23短程配对"
    assert 生成判断(mock, 20)["状态"] == "证据不足"
    print("G8自检通过：四组相关、均匀等价、Oracle指标和止损门槛正常")
...
def 解析参数() -> argparse.Namespace:
    ...
    parser.add_argument("--groups", default="4")
    ...
    parser.add_argument(
        "--output_dir",
        type=Path,
        default=Path("outputs/V23分组保真预聚合/G8_100样本诊断"),
    )
...
def 主程序(args: argparse.Namespace) -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("G8完整诊断需要CUDA运行V09冻结模型")
    ...
    logger = logging.getLogger("StereoDETR_G8_GPSD")
    ...
    progress = tqdm.tqdm(total=sample_count, desc="GPSD-G8")
