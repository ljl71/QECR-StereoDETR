...
def build_optimizer(cfg_optimizer, model):
    base_lr = float(cfg_optimizer['lr'])
    multiplier_cfg = cfg_optimizer.get('parameter_lr_multipliers', {})
    if multiplier_cfg is None:
        multiplier_cfg = {}
    if not isinstance(multiplier_cfg, dict):
        raise TypeError('optimizer.parameter_lr_multipliers must be a mapping')
    multipliers = {
        str(pattern): float(multiplier)
        for pattern, multiplier in multiplier_cfg.items()
    }
    if any(multiplier <= 0.0 for multiplier in multipliers.values()):
        raise ValueError('optimizer learning-rate multipliers must be positive')
    ...
    for name, param in model.named_parameters():
        ...
        matched = [
            multiplier for pattern, multiplier in multipliers.items()
            if pattern in normalized_name
        ]
        if len(matched) > 1:
            raise ValueError(
                'multiple learning-rate multipliers match {}'.format(name)
            )
        multiplier = matched[0] if matched else 1.0
        learning_rate = base_lr * multiplier
        weight_decay = 0.0 if 'bias' in normalized_name else float(
    ...
    parameters = [
        ...
        for (learning_rate, weight_decay), params in sorted(grouped.items())
    ]
