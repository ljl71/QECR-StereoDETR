...
def extract_dets_from_outputs(
    ...
    # In the guided path, candidate retrieval and final ranking use the same
    # score.  Keeping the switch off is bit-for-bit equivalent to the original
    # selection rule and therefore supplies a controlled ablation.
    selection_prob = prob
    if quality_guided_topk:
        if quality is None:
            raise ValueError(
    ...
    # Preserve the class score in the existing detection tensor.  The final
    # depth/quality factor is still multiplied exactly once by
    # decode_detections().
