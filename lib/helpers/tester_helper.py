...
class Tester(object):
    def __init__(self, cfg, model, dataloader, logger, train_cfg=None, model_name='monodetr'):
        ...
        self.writelist_train = dataloader.dataset.writelist_train
    ...
    def test(self):
        ...
        if self.cfg['mode'] == 'single' or not self.train_cfg["save_all"]:
            ...
            self.inference()
            if getattr(self.dataloader.dataset, 'split', None) == 'test':
                self.logger.info(
                    'KITTI test split has no public labels; skipping local evaluation.'
                )
            else:
                self.evaluate()
    ...
    def inference(self):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.dataloader):
            ...
            # flops = FlopCountAnalysis(self.model, (inputs, calibs, targets, img_sizes, img_sizes_ori, img_sizes_upper, ))
            # print(">>>>flops.total()>>>>>", flops.total())  # 总 FLOPs 数（单位为次浮点运算）

            # 打印参数统计表
            # print(">>>>parameter_count_table(model)>>>>>", parameter_count_table(self.model))
            ...
            dets = extract_dets_from_outputs(
                ...
                K=self.max_objs,
                topk=self.cfg['topk'],
                quality_guided_topk=bool(
                    self.cfg.get('quality_guided_topk', False)
                ),
            )
    ...
    def save_results(self, results):
        ...
        for img_id in results.keys():
            ...
            for i in range(len(results[img_id])):
                class_name = self.writelist_train[int(results[img_id][i][0])]
    ...
    def save_results_samples(self, results):
        output_dir = os.path.join(self.output_dir, 'outputs', 'data_samples')
        os.makedirs(output_dir, exist_ok=True)
