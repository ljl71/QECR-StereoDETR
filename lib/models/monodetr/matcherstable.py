...
class StableHungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, group_num=11):
        ...
        # Final cost matrix
        C = self.cost_bbox * cost_bbox + self.cost_3dcenter * cost_3dcenter + self.cost_class * cost_class + self.cost_giou * cost_giou + self.cost_depth * cost_depth
        ...
        for batch_index in range(bs):
            ...
            pre_filtered = indices_i[0][all_iou_indices >= 0.2]
            gt_filtered = indices_i[1][all_iou_indices >= 0.2]
            indices_filted.append([pre_filtered, gt_filtered])
        return indices_match, indices_filted


def build_Stablematcher(cfg):
