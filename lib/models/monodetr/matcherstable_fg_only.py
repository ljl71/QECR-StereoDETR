...
class StableHungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, group_num=11):
        ...
        for batch_index in range(bs):
            ...
            # scipy's Hungarian matcher returns CPU indices, while IoU is
            # computed on the model device. PyTorch 2.x requires the boolean
            # mask and the indexed tensor to live on the same device.
            filter_mask = (all_iou_indices >= self.iou_thresh).cpu()
            pre_filtered = indices_i[0][filter_mask]
            gt_filtered = indices_i[1][filter_mask]
