"""Sparse query-level epipolar consistency for QECR-StereoDETR.

The module deliberately avoids a second dense right-to-left cost volume.  It
samples only a short horizontal window around each object's current disparity
hypothesis, optionally performs a reverse check, and uses the resulting
confidence to refine the query's local depth distribution.
"""
...
class QueryEpipolarConsistencyRefiner(nn.Module):
    """Refine a local query-depth posterior with sparse epipolar matching.

    Coordinate conventions:

    * ``query_points`` are normalized to [0, 1] in the network input image.
    * ``stereo_fb`` is expressed in input-image pixels times metres and must be
      consistent with the depth scale used by the detector.
    * ``stereo_direction`` is -1 for the normal KITTI left-to-right ordering
      and +1 after a simultaneous horizontal flip or a left/right switch.
    """
    ...
    def _sample(
        ...
        """Sample ``[B, Q, K, 2]`` points and return ``[B, Q, K, C]``."""
    ...
    def _masked_probabilities(
        ...
        probabilities = F.softmax(
            masked_logits / self.matching_temperature,
            dim=-1,
        )
        ...
        confidence = 1.0 - _normalized_entropy(probabilities)
    ...
    def forward(
        ...
        initial_depth = (base_probabilities * local_bins).sum(
            dim=-1, keepdim=True
        ).clamp_min(1.0e-3)
        ...
        right_x = right_center + offsets_normalized
        right_y = points[..., 1:2].expand(-1, -1, self.num_samples)
        right_points = torch.stack([right_x, right_y], dim=-1)
        ...
        left_query_norm = F.normalize(left_query, dim=-1, eps=1.0e-6)
        right_candidates_norm = F.normalize(
            right_candidates, dim=-1, eps=1.0e-6
        )
        forward_logits = (
            right_candidates_norm * left_query_norm.unsqueeze(2)
        ).sum(dim=-1)
        ...
        epipolar_depth = (
            fb / refined_disparity_pixels.clamp_min(self.min_disparity_pixels)
        )
        ...
        if self.bidirectional:
            ...
            reverse_center = refined_right_x - direction * disparity_normalized
            left_x = reverse_center + offsets_normalized
            left_y = points[..., 1:2].expand(-1, -1, self.num_samples)
            left_points = torch.stack([left_x, left_y], dim=-1)
            left_valid = self._valid_points(left_points)
            left_candidates = self._sample(left_features, left_points)
            right_query_norm = F.normalize(right_query, dim=-1, eps=1.0e-6)
            left_candidates_norm = F.normalize(
                left_candidates, dim=-1, eps=1.0e-6
            )
            reverse_logits = (
                left_candidates_norm * right_query_norm.unsqueeze(2)
            ).sum(dim=-1)
        ...
        cycle_confidence = F.softplus(
            threshold - cycle_error
        ) / F.softplus(threshold)
        cycle_confidence = cycle_confidence.clamp(0.0, 1.0)
