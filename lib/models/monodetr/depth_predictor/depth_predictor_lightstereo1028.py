...
class LightStereoDepthPredictor(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.upsample = nn.Sequential(
            nn.Conv2d(48, 256, kernel_size=(1, 1)),
            nn.GroupNorm(32, 256))
    ...
    def forward(self, feature_stereo, mask, pos, targets=None):
        ...
        src_8 = self.downsample(feature_stereo[0][:batch_size_half])
        if self.training:
            gwc_volume_list = []
            for  batch_id in range(batch_size_half):
                flip_flag = targets[batch_id]["random_flip_flag"]
                f_left_i = feature_stereo[0][batch_id].unsqueeze(0)
                f_right_i = feature_stereo[0][batch_id+batch_size_half].unsqueeze(0)
                if flip_flag:
                    gwc_volume_list.append(correlation_volume_flip(f_left_i, f_right_i, 192 // 4))
                else:
                    gwc_volume_list.append(correlation_volume(f_left_i, f_right_i, 192 // 4))
            gwc_volume = torch.cat(gwc_volume_list, 0)
        else:            
            gwc_volume = correlation_volume(feature_stereo[0][:batch_size_half], feature_stereo[0][batch_size_half:], 192 // 4)
        features_left = []
