...
class Aggregation(nn.Module):
    def __init__(self, in_channels, in_channels_s8, in_channels_s16, left_att, blocks, expanse_ratio, backbone_channels):
        ...
        self.conv5 = nn.Sequential(
            nn.ConvTranspose2d(in_channels * 4 + in_channels_s16, 
                               in_channels * 2 + in_channels_s8, 
                               3, padding=1, output_padding=1, stride=2, bias=False),
            nn.BatchNorm2d(in_channels * 2 + in_channels_s8))

        self.conv6 = nn.Sequential(
            nn.ConvTranspose2d(in_channels * 2 + in_channels_s8, in_channels, 3, padding=1, output_padding=1, stride=2, bias=False),
            nn.BatchNorm2d(in_channels))

        self.redir1 = MobileV2Residual(in_channels, in_channels, stride=1, expanse_ratio=self.expanse_ratio)
        self.redir2 = MobileV2Residual(in_channels * 2 + in_channels_s8, 
                                       in_channels * 2 + in_channels_s8, stride=1, expanse_ratio=self.expanse_ratio)
...
class LightStereoDepthPredictor(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.upsample = nn.Sequential(
            nn.Conv2d(24, 256, kernel_size=(1, 1)),
            nn.GroupNorm(32, 256))
    ...
    def forward(self, feature_stereo, mask, pos, targets=None):
        ...
        if self.training:
            ...
            for  batch_id in range(batch_size_half):
                ...
                if flip_flag:
                    ...
                    gwc_volume_list8.append(correlation_volume(f_left_i_s8, f_right_i_s8, 192 // 8))
        ...
        encoding_volume = self.cost_agg(gwc_volume_s4, gwc_volume_s8, gwc_volume_s16, features_left)  # [bz, 1, max_disp/4, H/4, W/4]
        squeezed_encoding = encoding_volume[0].reshape(encoding_volume[0].size(0), 
                                                       -1, 
                                                       encoding_volume[0].size(2), 
                                                       encoding_volume[0].size(3))  # [bz, max_disp/4, H/4, W/4]
        PSV_features = squeezed_encoding
        PSV_features = self.upsample(PSV_features)
        PSV_features_inter = F.interpolate(PSV_features, size=src_16.shape[-2:], mode='bilinear')
        # features_for_depth = (PSV_features_inter + src_16_left + src_8)/3
