...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        self.disparity_supervision_cfg = cfg.get(
            'disparity_supervision', {}
        ) or {}
        self.disparity_target_mode = self.disparity_supervision_cfg.get(
            'mode', 'scalar'
        )
        if self.disparity_target_mode not in ['scalar', 'multi_candidate']:
            raise ValueError(
                "dataset.disparity_supervision.mode must be 'scalar' or "
                "'multi_candidate'"
            )
        self.disparity_cell_size = int(
            self.disparity_supervision_cfg.get('cell_size', 4)
        )
        if self.disparity_cell_size != 4:
            raise ValueError(
                'StereoDETR disparity supervision currently requires a 4x4 cell'
            )
        self.disparity_lrc_soft_weight = bool(
            self.disparity_supervision_cfg.get('lrc_soft_weight', False)
        )
        self.disparity_lrc_temperature = float(
            self.disparity_supervision_cfg.get('lrc_temperature', 1.0)
        )
        self.disparity_lrc_confidence_floor = float(
            self.disparity_supervision_cfg.get('lrc_confidence_floor', 0.25)
        )
        if self.disparity_lrc_temperature <= 0:
            raise ValueError('lrc_temperature must be positive')
        if not 0.0 <= self.disparity_lrc_confidence_floor <= 1.0:
            raise ValueError('lrc_confidence_floor must be in [0, 1]')
        self.use_multi_candidate_disparity = (
            self.disparity_target_mode == 'multi_candidate'
            and split in ['train', 'trainval', 'train_self_val']
        )
        if self.use_multi_candidate_disparity:
            if not self.get_disparity_on_the_fly:
                raise ValueError(
                    'multi-candidate disparity requires '
                    'get_disparity_on_the_fly=True'
                )
            if self.with_disparity_noise != 0:
                raise ValueError(
                    'with_disparity_noise must be 0 for multi-candidate '
                    'supervision so scalar and candidate targets stay aligned'
                )
        ...
        teacher_cache_cfg = cfg.get('teacher_cache', {})
        self.teacher_cache_enabled = bool(
            teacher_cache_cfg.get('enabled', False)
            and split in ['train', 'trainval', 'train_self_val']
        )
        self.teacher_cache_root = os.path.expanduser(
            teacher_cache_cfg.get('root_dir', '')
        )
        self.teacher_cache_strict = bool(
            teacher_cache_cfg.get('strict', True)
        )
        if self.teacher_cache_enabled:
            if not self.teacher_cache_root:
                raise ValueError(
                    "teacher_cache.root_dir is required when teacher cache is enabled"
                )
            if self.random_mixup3d > 0 or self.random_switch > 0:
                raise ValueError(
                    "offline teacher cache requires random_mixup3d=0 and "
                    "random_switch=0 to preserve pixel alignment"
                )
    ...
    def get_teacher_cache(self, idx):
        """Load full-image metric depth and confidence from a compressed cache."""
        cache_file = os.path.join(self.teacher_cache_root, idx + '.npz')
        if not os.path.isfile(cache_file):
            if self.teacher_cache_strict:
                raise FileNotFoundError(
                    "missing offline teacher cache: {}".format(cache_file)
                )
            return None, None

        with np.load(cache_file, allow_pickle=False) as cache:
            if 'depth' not in cache or 'confidence' not in cache:
                raise KeyError(
                    "{} must contain depth and confidence arrays".format(
                        cache_file
                    )
                )
        ...
        if depth.ndim != 2 or confidence.shape != depth.shape:
            raise ValueError(
                "invalid teacher cache shapes in {}: depth={}, confidence={}".format(
                    cache_file, depth.shape, confidence.shape
                )
            )
        ...
        return Image.fromarray(depth, mode='F'), Image.fromarray(confidence, mode='F')
    
    @staticmethod
    def _create_cv2_matcher(matcher_mode, right_matcher=False):
        max_disparity = 192
        min_disparity = -max_disparity if right_matcher else 0
        if matcher_mode == "StereoBM":
            matcher = cv2.StereoBM_create(max_disparity, 25)
            matcher.setMinDisparity(min_disparity)
            return matcher
        if matcher_mode == "SGBM":
            block_size = 5
        ...
        raise ValueError(
            "matcher_mode must be 'StereoBM' or 'SGBM', got {}".format(
                matcher_mode
            )
        )

    @staticmethod
    def _left_right_soft_confidence(
        disparity_left,
        disparity_right,
        temperature,
        confidence_floor,
    ):
        """Convert left-right residual into a non-destructive soft weight."""
        height, width = disparity_left.shape
        left_valid = (
            np.isfinite(disparity_left)
            & (disparity_left > 0.0)
            & (disparity_left < 192.0)
        )
        right_x = np.rint(
            np.arange(width, dtype=np.float32)[None, :]
            - np.nan_to_num(disparity_left, nan=0.0)
        ).astype(np.int64)
        in_bounds = left_valid & (right_x >= 0) & (right_x < width)
        safe_right_x = np.clip(right_x, 0, width - 1)
        rows = np.arange(height, dtype=np.int64)[:, None]
        matched_right = disparity_right[rows, safe_right_x]
        right_valid = (
            np.isfinite(matched_right)
            & (matched_right < 0.0)
            & (matched_right > -192.0)
        )
        paired = in_bounds & right_valid
        confidence = np.zeros_like(disparity_left, dtype=np.float32)
        confidence[left_valid] = confidence_floor
        residual = np.abs(disparity_left + matched_right)
        confidence[paired] = confidence_floor + (
            1.0 - confidence_floor
        ) * np.exp(-residual[paired] / temperature)
        return np.clip(confidence, 0.0, 1.0).astype(np.float32)

    @staticmethod
    def _pack_disparity_cells(disparity_fixed, confidence, cell_size=4):
        """Pack each full-resolution 4x4 cell into 16 candidate channels."""
        height, width = disparity_fixed.shape
        grid_height = (height + cell_size - 1) // cell_size
        grid_width = (width + cell_size - 1) // cell_size
        padded_shape = (grid_height * cell_size, grid_width * cell_size)
        padded_disp = np.zeros(padded_shape, dtype=np.uint16)
        padded_weight = np.zeros(padded_shape, dtype=np.float32)
        padded_disp[:height, :width] = disparity_fixed
        padded_weight[:height, :width] = confidence
        candidates = (
            padded_disp.reshape(
                grid_height, cell_size, grid_width, cell_size
            )
            .transpose(1, 3, 0, 2)
            .reshape(cell_size * cell_size, grid_height, grid_width)
        )
        candidate_weights = (
        ...
        scalar = candidates.max(axis=0)
        return scalar, candidates, candidate_weights
    ...
    def get_disparty_P2_onfly(self, img_left, img_right, flip_flag, switch_flag):
        ...
        matcher = self._create_cv2_matcher(self.matcher_mode)
        needs_right_match = (
            self.use_multi_candidate_disparity
            and self.disparity_lrc_soft_weight
        )
        if needs_right_match:
            right_matcher = self._create_cv2_matcher(
                self.matcher_mode, right_matcher=True
            )
        if switch_flag or flip_flag:
            gray_image1_flip = cv2.flip(gray_image1, 1)
            gray_image2_flip = cv2.flip(gray_image2, 1)
            disparity_left_raw = matcher.compute(
                gray_image1_flip, gray_image2_flip
            )
            if needs_right_match:
                disparity_right_raw = right_matcher.compute(
                    gray_image2_flip, gray_image1_flip
                )
        else :
            disparity_left_raw = matcher.compute(gray_image1, gray_image2)
            if needs_right_match:
                disparity_right_raw = right_matcher.compute(
                    gray_image2, gray_image1
                )

        disparity_left_px = disparity_left_raw.astype(np.float32) / 16.0
        left_valid = (
            np.isfinite(disparity_left_px)
            & (disparity_left_px > 0.0)
            & (disparity_left_px < 192.0)
        )
        if needs_right_match:
            disparity_right_px = (
                disparity_right_raw.astype(np.float32) / 16.0
            )
            confidence = self._left_right_soft_confidence(
                disparity_left_px,
                disparity_right_px,
                self.disparity_lrc_temperature,
                self.disparity_lrc_confidence_floor,
            )
        else:
            confidence = left_valid.astype(np.float32)

        disparity_left_fixed = np.where(
            left_valid, disparity_left_raw, 0
        ).astype(np.uint16)
        if switch_flag or flip_flag:
            disparity_left_fixed = np.ascontiguousarray(
                disparity_left_fixed[:, ::-1]
            )
            confidence = np.ascontiguousarray(confidence[:, ::-1])

        scalar, candidates, candidate_weights = self._pack_disparity_cells(
            disparity_left_fixed,
            confidence,
            self.disparity_cell_size,
        )
        if self.use_multi_candidate_disparity:
            return scalar, candidates, candidate_weights
        return scalar
    ...
    def __getitem__(self, item):
        ...
        disp_candidate_weights = None
        if self.teacher_cache_enabled:
            teacher_depth, teacher_confidence = self.get_teacher_cache(index)
        else:
            teacher_depth, teacher_confidence = None, None
        ...
        if self.data_augmentation:
            ...
            if np.random.random() < self.random_flip:
                ...
                if teacher_depth is not None:
                    teacher_depth = teacher_depth.transpose(Image.FLIP_LEFT_RIGHT)
                    teacher_confidence = teacher_confidence.transpose(
        ...
        if self.teacher_cache_enabled:
            if teacher_depth is None:
                empty_shape = (
                    int(img_size_original[1]),
                    int(img_size_original[0]),
                )
                teacher_depth = Image.fromarray(
                    np.zeros(empty_shape, dtype=np.float32), mode='F'
                )
                teacher_confidence = Image.fromarray(
                    np.zeros(empty_shape, dtype=np.float32), mode='F'
                )
            teacher_depth = teacher_depth.resize(
                tuple(img_size_original.tolist()), resample=Image.BILINEAR
            )
            teacher_confidence = teacher_confidence.resize(
                tuple(img_size_original.tolist()), resample=Image.BILINEAR
            )
            teacher_depth = teacher_depth.transform(
                tuple(self.resolution_ori.tolist()),
                method=Image.AFFINE,
                data=tuple(trans_inv.reshape(-1).tolist()),
                resample=Image.BILINEAR,
            )
            teacher_confidence = teacher_confidence.transform(
                tuple(self.resolution_ori.tolist()),
                method=Image.AFFINE,
                data=tuple(trans_inv.reshape(-1).tolist()),
                resample=Image.BILINEAR,
            ...
            teacher_depth = teacher_depth.resize(
                tuple(self.resolution_disp_ori.tolist()),
                resample=Image.BILINEAR,
            )
            teacher_confidence = teacher_confidence.resize(
                tuple(self.resolution_disp_ori.tolist()),
        ...
        if self.get_disparity and self.get_disparity_on_the_fly:
            disparity_result = self.get_disparty_P2_onfly(
                img, img3, random_flip_flag, random_switch_flag
            )
            if isinstance(disparity_result, tuple):
                (
                    disp_P2,
                    disp_candidates,
        ...
        if disp_candidates is not None:
            disp_candidates = (
                np.asarray(disp_candidates, dtype=np.float32) / 16.0
            )
            disp_candidate_weights = np.asarray(
                disp_candidate_weights, dtype=np.float32
            )

        if self.teacher_cache_enabled:
            teacher_depth = np.asarray(teacher_depth, dtype=np.float32)
            teacher_confidence = np.asarray(
                teacher_confidence, dtype=np.float32
            )
            if self.depth_scale == 'normal':
                teacher_depth = teacher_depth * crop_scale
            elif self.depth_scale == 'inverse':
                teacher_depth = teacher_depth / crop_scale
            if self.crop_top > 0:
                teacher_upper = self.crop_top // 4
                teacher_depth = teacher_depth[teacher_upper:, :]
                teacher_confidence = teacher_confidence[teacher_upper:, :]
            teacher_depth = np.expand_dims(teacher_depth, axis=0)
            teacher_confidence = np.expand_dims(
                teacher_confidence, axis=0
            )
        ...
        if self.crop_top > 0:
            ...
            if disp_candidates is not None:
                disparity_upper = self.croptop_func.upper // 4
                disp_candidates = disp_candidates[:, disparity_upper:, :]
                disp_candidate_weights = disp_candidate_weights[
                    :, disparity_upper:, :
                ]
        ...
        targets = {
                   ...
                   'img_id': index}
        if self.teacher_cache_enabled:
            targets['teacher_depth'] = teacher_depth
            targets['teacher_confidence'] = teacher_confidence
        if disp_candidates is not None:
            targets['disp_candidates'] = disp_candidates
            targets['disp_candidate_weights'] = disp_candidate_weights
        info = {'img_id': index,
                'img_size_original': img_size_original,
                'img_size_croped': img_size_croped,
                "upper": self.croptop_func.upper,
